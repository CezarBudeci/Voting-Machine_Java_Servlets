					...
					<div class="col-md-12 mainMainColumn">
						<article class="forMainBox">
							<div class="mainBox">
								...
								<form class="row formRow" method="POST" action="storecode">
								<div class="row">
									<div class="col-md-12">
									...
									<div class="col-md-6 inputColumns">
										<input class="form-control emailInput" type="text" name="emailAddress" placeholder="Type an email here...">
									</div>
								...
								<div class="row">
									<div class="col-md-12">
										...
										for(CountOfCand nrOfCand : nrOfCands){
											out.print(nrOfCand.getCountNr());
										}
										%></p>
									</div>
								</div>
									<% List<CandidateObj> candidateList =  (ArrayList<CandidateObj>) request.getAttribute("allCands");
										int i = 1;
										for (CandidateObj individualCand : candidateList){
											out.print("<div class='row politicianRow'>");
											out.print("<div class='col-md-6 politicianCols'>");
											out.print("<a class='politicianParag' href='/ProfilePage?id="+individualCand.getId()+"' method='POST'>" + i + ". " + individualCand.getFname() + " " + individualCand.getLname() + "</a>");
											out.print("</div>");
											out.print("<div class='col-md-6 buttonsColumns1 delBtnCol1'>");
											out.print("<button type='button' class='btn deleteBtn1' data-bs-toggle='modal' data-bs-target='#exampleModal" + individualCand.getId() + "'>delete</button>");
											out.print("</div>");
											out.print("</div>");


											out.print("<div class='modal fade' id='exampleModal" + individualCand.getId() + "' tabindex='-1' aria-labelledby='exampleModalLabel' aria-hidden='true'>");
											        ...
											        out.print("<form action='deletecandidate' method='POST' id='deleteACand" + individualCand.getId() + "'>");
											        out.print("<p>");
											        out.print("Are you sure you want to delete " + individualCand.getFname() + " " + individualCand.getLname() + " from the list?");
											        out.print("</p>");
											        out.print("<p>");
											        out.print("This action cannot be undone!");
											        out.print("</p>");
											        out.print("<input class='inputToHide' name='candidateIdDel' value='" + individualCand.getId() + "' hidden/>");
											        out.print("</form>");
											        out.print("</div>");
											        		out.print("<div class='modal-footer'>");
											        				out.print("<button type='button' class='btn btn-secondary' data-bs-dismiss='modal'>Close</button>");
											        						out.print("<button type='submit' class='btn deleteBtn1' form='deleteACand" + individualCand.getId() + "'>Delete</button>");
											        								out.print("</div>");
											        										out.print("</div>");
											        												out.print("</div>");
											        														out.print("</div>");
